    ...
    def run_obc_nn(self, dt, samp, nsampes, prev_cols,
        ...
        for it in range(1, nsteps + 1):
            ...
            for fld, bm1_n, b_n, ext_all, fcode in (
                ...
                # For using the Neural Network only for h and update u,v using orlanski
                dphi = 0.0                                                          # [OBC-corr] boundary-value correction (0 = none)
                if phase_fn is not None and fcode == 0:
                    _out = phase_fn(Xf, fcode)                                      # [OBC-corr] (rx,ry) OR (drx,dry,dphi_phys)
                    if len(_out) == 3:                                              # [OBC-corr] correction mode: analytic + NN correction
                        _drx, _dry, dphi = _out
                        rx = np.clip(rx + _drx, 0.0, 1.0)                           # [OBC-corr] CFL-clamp the combined r
                        ry = np.clip(ry + _dry, -1.0, 1.0)
                    else:                                                           # [OBC-corr] back-compat: direct (rx,ry) override
                        rx, ry = _out
                upd = _obc_update_np(Xf, rx, ry, inflow, ext_all[it-1].reshape(-1), alpha_eff) + dphi  # [OBC-corr] +dphi
