...
class SSWEM:
    ...
    def run_record_bc(self, dt, samp, nsamps, bc_cols,
                      ...
                      store_centerline=False, j_center=None):   # [CL] optional per-step zonal center-line store
        """
        dt      - Time step [s]
        samp    - Steps between samples [steps]
        ...
        [CL] Optional zonal center-line store (off by default):
        store_centerline - if True, ALSO store (u,v,h) every step along the whole
                  zonal row j_center (all i). Cheap: (nsteps, nk, ni) per field.
        j_center - row index of the line (default self.nj//2, the domain center).
        ...
        are False). With store_downstream it has 'cols','x_km','h'/'u'/'v'
        (each (nsteps, nk, nj, n_probe)); with store_centerline it also has 'j_center',
        'x_km_h'/'x_km_u', 'y_km_h'/'y_km_v', and 'h_cl'/'u_cl'/'v_cl' (each
        (nsteps, nk, ni)). The stores are read from the post-step state; downstream of
        the sponge the restoring is zero there, so that equals the pre-restoring phase
        used for the boundary stores.
        """
        ...
        # [CL] center-line setup: store the whole zonal row jc (all i) every step.
        # [CL] jc defaults to the domain center; h/u share y=yh1[jc], v is at y=yq1[jc].
        if store_centerline:
            jc = self.nj // 2 if j_center is None else int(j_center)
            if not (0 <= jc < self.nj):
                raise ValueError(f"[CL] j_center {jc} out of range [0, {self.nj})")
            h_cl_all = np.zeros((nsteps, self.nk, self.ni))    # [CL] (nsteps, nk, ni)
            u_cl_all = np.zeros((nsteps, self.nk, self.ni))    # [CL]
            v_cl_all = np.zeros((nsteps, self.nk, self.ni))    # [CL]
        ...
        for iter in range( 1, nsteps + 1 ):
            ...
            if store_centerline:                             # [CL] post-step zonal center line (all i at row jc)
            ...
            if np.any( np.isnan( self.u ) ):
                ...
                if store_centerline:                         # [CL] trim center line too
        ...
        probe = None
        if store_downstream or store_centerline:             # [CL] probe holds either/both diagnostics
            probe = {}
        if store_downstream:
        ...
        if store_centerline:                                 # [CL] zonal center-line stores + per-field x/y axes
            probe.update({ 'j_center': jc,
                           'x_km_h': self.xh1 / 1e3, 'x_km_u': self.xq1 / 1e3,   # [CL] h,v on xh1; u on xq1
