...
def _orlanski_east(phi, phi_prev, b_obc, rx_out,
                   ...
                   rxy_out):  # [OBC - 2D - PhaseDiag] +rxy_out (nk,nj,2):
    """
    ...
    for k in range(nk):
        for j in range(nj):
            ...
            jm = j - 1 if j > 0 else 0       # [NP] mirror ghost (south wall)
            jp = j + 1 if j < nj - 1 else nj - 1 # [NP] mirror ghost (north wall)
            ...
            # fix the 0/0 division problem 
            eps_denom = 1e-5                                        
            r_d = (1.0 / eps_denom) if denom < eps_denom else (1.0 / denom) 
            rx = -dphi_t * dphi_x * r_d                             
            ry = -dphi_t * dphi_y * r_d                             

            if rx < 0.0: # inflow
                rx = 0.0
                ry = 0.0   # [OBC - 2D - PhaseDiag] inflow is prescribed/nudged, not radiated -> record (rx,ry)=(0,0)
                if nudging_mode == 1:
                    ...
                    phi[k, j, b_obc+2] = phi[k, j, b_obc+1]
                else:
...
def _step_numba(u, v, h, D, taux, tauy, f, f_at_u, f_at_v,
                ...
                rxy_h, rxy_u, rxy_v,                          # [OBC - 2D - PhaseDiag] per-row (nk,nj,2) applied (rx,ry) for h,u,v
    ...
    for sweep in range(2):
        if ( sweep == 0 ) == do_i_first:
            ...
            for k in range(nk):
                for j in prange(nj):
                    jm = j - 1 if j > 0 else 0       # [NP] wall: hv[0] uses v[0]=0 -> zero flux through south wall
                    for i in range(ni):
                        vi = v[k,j,i]
            ...
            for k in range(nk):
                for j in prange(nj):
                    for i in range(ni):
                        hv_jp = 0.0 if j == nj - 1 else hv[k,j+1,i]   # [NP] north wall: zero meridional flux (was periodic wrap)
    ...
    if obc_on == 1:
        if h_zerograd == 1:
            ...
            _orlanski_east(h, h_prev, b_obc, rx_h,
                           ...
                           rxy_h)  #[OBC - 2D - PhaseDiag] +per-row rxy_h
    ...
    for k in range(nk):
        for j in prange(nj):
            jm = j - 1 if j > 0 else 0           # [NP] free-slip N/S wall: mirror ghost row (was periodic)
            for i in range(ni):
                im = i - 1 if i > 0 else ni - 1
                ip = i + 1 if i < ni - 1 else 0
                ...
                vjp = 0.0 if j == nj - 1 else v[k,j+1,i]; vn_jp = vjp if vjp < 0.0 else 0.0  # [NP] v=0 on north wall
    ...
    for k in range(nk):
        for j in prange(nj):
            jm = j - 1 if j > 0 else 0            # [NP] free-slip wall: mirror ghost for h/u/f (was periodic)
            jp = j + 1 if j < nj - 1 else nj - 1  # [NP] mirror ghost; v & hv across north face use the wall-zero locals below
            for i in range(ni):
                im = i - 1 if i > 0 else ni - 1
                ip = i + 1 if i < ni - 1 else 0
                at_n = ( j == nj - 1 )                   # [NP] north wall row -> v=0, hv=0 across the north face
                v_jp_i   = 0.0 if at_n else v[k,j+1,i]   # [NP]
                v_jp_im  = 0.0 if at_n else v[k,j+1,im]  # [NP]
                hv_jp_i  = 0.0 if at_n else hv[k,j+1,i]  # [NP]
                hv_jp_im = 0.0 if at_n else hv[k,j+1,im] # [NP]
    ...
    for k in range(nk):
        for j in prange(nj):
            ...
            for i in range(ni):
                im = i - 1 if i > 0 else ni - 1
                ip = i + 1 if i < ni - 1 else 0
                vdot_jp_i  = 0.0 if j == nj - 1 else vdot[k,j+1,i]    # [NP] v-tendency = 0 on north wall
                vdot_jp_im = 0.0 if j == nj - 1 else vdot[k,j+1,im]   # [NP]
...
# ===================== Lifted (numpy) east-Orlanski OBC for the NN path ===================== # [ROLLOUT]
# Plain-numpy mirror of THIS module's in-step _orlanski_east conventions (eps_denom=1e-5,        # [ROLLOUT]
# rx clip [0,3], ry clip [-3,3]).  Used by run_obc_nn, which LIFTS the OBC out of the @njit      # [ROLLOUT]
# step so a torch network can supply (rx, ry) -- torch cannot run inside numba nopython.         # [ROLLOUT]
def _phase_inflow_np(X8):  # [ROLLOUT]
    """Analytic per-point (rx, ry) + inflow flag from the packed stencil (numpy).  # [ROLLOUT]
    Faithful copy of the estimation branch of _orlanski_east; inflow == (raw rx < 0)."""  # [ROLLOUT]
    pim1, pim2            = X8[..., 6], X8[..., 7]                                # [ROLLOUT]
    bm1_jm, bm1_j, bm1_jp = X8[..., 0], X8[..., 1], X8[..., 2]                    # [ROLLOUT]
    dphi_t = pim1 - bm1_j                                                          # [ROLLOUT]
    dphi_x = pim1 - pim2                                                           # [ROLLOUT]
    cen    = bm1_jp - bm1_jm                                                       # [ROLLOUT]
    dphi_y = np.where(dphi_t * cen > 0.0, bm1_j - bm1_jm, bm1_jp - bm1_j)          # [ROLLOUT] upwind in j
    denom  = dphi_x * dphi_x + dphi_y * dphi_y                                     # [ROLLOUT]
    eps_denom = 1e-5                                     # [ROLLOUT] matches this module's njit (not 1e-3)
    r_d = 1.0 / np.maximum(denom, eps_denom)             # [ROLLOUT] vectorized small-denominator guard
    rx = -dphi_t * dphi_x * r_d                          # [ROLLOUT]
    ry = -dphi_t * dphi_y * r_d                          # [ROLLOUT]
    inflow = rx < 0.0                                    # [ROLLOUT] inflow flag from RAW rx (before clip)
    rx = np.clip(rx, 0.0, 3.0)                           # [ROLLOUT] matches njit clip (rx <= 3)
    ry = np.clip(ry, -3.0, 3.0)                          # [ROLLOUT] matches njit clip (|ry| <= 3)
    return rx, ry, inflow                                # [ROLLOUT]


def _obc_update_np(X8, rx, ry, inflow, phi_ext, alpha_in):  # [ROLLOUT]
    """East-Orlanski boundary update (numpy), faithful to _orlanski_east.  # [ROLLOUT]
    OUTFLOW: phi_b = (phi_prev_b + rx*pim1 - ry*dy_b)/(1+rx).  # [ROLLOUT]
    INFLOW : phi_b = phi_prev_b + alpha_in*(phi_ext - phi_prev_b)  (rx,ry unused)."""  # [ROLLOUT]
    phi_prev_b_j = X8[..., 4]    # (b, j)   @ n                                    # [ROLLOUT]
    pim1         = X8[..., 6]    # (b-1, j) @ n+1                                  # [ROLLOUT]
    b_jm         = X8[..., 3]    # (b, j-1) @ n                                    # [ROLLOUT]
    b_jp         = X8[..., 5]    # (b, j+1) @ n                                    # [ROLLOUT]
    dy_b  = np.where(ry >= 0.0, phi_prev_b_j - b_jm, b_jp - phi_prev_b_j)          # [ROLLOUT]
    rad   = (phi_prev_b_j + rx * pim1 - ry * dy_b) / (1.0 + rx)                    # [ROLLOUT]
    nudge = phi_prev_b_j + alpha_in * (phi_ext - phi_prev_b_j)                     # [ROLLOUT]
    return np.where(inflow, nudge, rad)                                            # [ROLLOUT]
...
class SSWEM:
    ...
    def __init__(self, ni, g, Ho, Lx, fo, beta, epsilon, nu_h, nu_v=0,
        ...
        self.f_at_u = self.fo + self.beta * self.yu  # [NP] analytic f at u-points (was periodic q2u j-average; identical in interior, removes the north-wall wrap)
    ...
    def run_obc(self, dt, samp, nsampes, prev_cols,
        ...
        # [OBC - 2D - PhaseDiag] per-row, per-step APPLIED (rx,ry) for each field: (nsteps,nk,nj,2). [...,0]=rx, [...,1]=ry
        ...
        rx = {'t_step': np.arange(1, nrun+1) * dt,
                ...
                # [OBC - 2D - PhaseDiag] per-row APPLIED phase-speed Courant numbers, each (nrun,nk,nj).
                # Convert to phase speed [m/s]: c_x = rx*dx/dt, c_y = ry*dy/dt.
    ...
    # ------------------------------------------------------------------------------ # [ROLLOUT]
    #  Out-of-step NN driver: like run_obc, but the east phase speed (rx, ry) can     # [ROLLOUT]
    #  come from a neural network `phase_fn` (torch cannot run inside numba, so the   # [ROLLOUT]
    #  OBC is lifted out of the njit step).  Ported from NP_OSSWEM_orlanski_2D_NN     # [ROLLOUT]
    #  and adapted to THIS module's conventions: eps_denom=1e-5, rx/ry clip +-3,      # [ROLLOUT]
    #  ghost copies col b -> b+1 -> b+2 after the boundary update.                    # [ROLLOUT]
    # ------------------------------------------------------------------------------ # [ROLLOUT]
    def run_obc_nn(self, dt, samp, nsampes, prev_cols,                                # [ROLLOUT]
                   h_bc_all, u_bc_all, v_bc_all, b_obc,                               # [ROLLOUT]
                   h_ext_all, u_ext_all, v_ext_all,                                   # [ROLLOUT]
                   phase_fn=None, nudging=True, alpha_in=0.99):                       # [ROLLOUT]
        """West: prescribe stored cols.  East: out-of-step Orlanski at col b_obc;     # [ROLLOUT]
        phase_fn(Xf, fcode) -> (rx, ry) overrides the analytic estimate for h only    # [ROLLOUT]
        (fcode 0); u, v keep the analytic (rx, ry).  phase_fn=None -> all analytic    # [ROLLOUT]
        (out-of-step classical baseline).  Returns (u, v, h, time, diffs)."""         # [ROLLOUT]
        prev_cols = np.ascontiguousarray(np.asarray(prev_cols, dtype=np.int64).ravel())  # [ROLLOUT]
        b = int(b_obc)                                                                 # [ROLLOUT]
        alpha_eff = float(alpha_in) if nudging else 1.0    # [ROLLOUT] False -> hard prescribe
        nsteps = nsampes * samp                                                        # [ROLLOUT]
        if h_bc_all.shape[0] < nsteps:                                                 # [ROLLOUT]
            raise ValueError(f"[ROLLOUT] stored west data has {h_bc_all.shape[0]} < nsteps={nsteps}")  # [ROLLOUT]

        def _prep_ext(a, name):                                                        # [ROLLOUT]
            a = np.asarray(a, dtype=np.float64)                                        # [ROLLOUT]
            if a.ndim == 4 and a.shape[-1] == 1:                                       # [ROLLOUT]
                a = a[..., 0]                                                          # [ROLLOUT]
            if a.ndim != 3 or a.shape[1:] != (self.nk, self.nj):                       # [ROLLOUT]
                raise ValueError(f"[ROLLOUT] {name} must be (>=nsteps,nk,nj[,1]); got {a.shape}")  # [ROLLOUT]
            if a.shape[0] < nsteps:                                                    # [ROLLOUT]
                raise ValueError(f"[ROLLOUT] {name} has {a.shape[0]} < nsteps={nsteps}")  # [ROLLOUT]
            return np.ascontiguousarray(a)                                             # [ROLLOUT]
        h_ext_all = _prep_ext(h_ext_all, "h_ext_all")                                  # [ROLLOUT]
        u_ext_all = _prep_ext(u_ext_all, "u_ext_all")                                  # [ROLLOUT]
        v_ext_all = _prep_ext(v_ext_all, "v_ext_all")                                  # [ROLLOUT]

        if not (2 <= b < self.ni - 2):                     # [ROLLOUT] b+2 ghost copy needs 2 cols east of b
            raise ValueError(f"[ROLLOUT] b_obc={b} needs 2 <= b < ni-2={self.ni-2}")   # [ROLLOUT]
        if np.any((prev_cols >= b - 2) & (prev_cols <= b)):                            # [ROLLOUT]
            raise ValueError(f"[ROLLOUT] prescribed cols must avoid {{{b-2},{b-1},{b}}}; got {list(prev_cols)}")  # [ROLLOUT]

        self._print_run_info(dt, nsteps)                                               # [ROLLOUT]
        print(f"[ROLLOUT] east phase speed from "                                      # [ROLLOUT]
              f"{'analytic numpy (classical baseline)' if phase_fn is None else 'NEURAL NETWORK (h only)'}; "  # [ROLLOUT]
              f"inflow {('NUDGE alpha_in=%.3g' % alpha_eff) if nudging else 'PRESCRIBE (hard)'}")  # [ROLLOUT]
        print("Running (Neural-Network-OBC, non-periodic N/S walls)...")               # [ROLLOUT]

        u = np.zeros((nsampes+1, self.nk, self.nj, self.ni))                           # [ROLLOUT]
        v = np.zeros((nsampes+1, self.nk, self.nj, self.ni))                           # [ROLLOUT]
        h = np.zeros((nsampes+1, self.nk, self.nj, self.ni))                           # [ROLLOUT]
        time = np.zeros((nsampes+1))                                                   # [ROLLOUT]
        u[0] = self.u; v[0] = self.v; h[0] = self.h; time[0] = self.time               # [ROLLOUT]

        h_diff = np.zeros(2); u_diff = np.zeros(2); v_diff = np.zeros(2)               # [ROLLOUT] west misfit scratch

        # [NP] tangential neighbours via free-slip mirror-clamp (south/north walls), NOT np.roll.  # [ROLLOUT]
        def jm(A):   # (.., j-1); at j=0 mirror the wall row                           # [ROLLOUT]
            out = np.empty_like(A); out[..., 1:] = A[..., :-1]; out[..., 0] = A[..., 0]; return out  # [ROLLOUT]
        def jp(A):   # (.., j+1); at j=nj-1 mirror the wall row                        # [ROLLOUT]
            out = np.empty_like(A); out[..., :-1] = A[..., 1:]; out[..., -1] = A[..., -1]; return out  # [ROLLOUT]
        def _X8(bm1_n, b_n, bm1_np1, bm2_np1):             # [ROLLOUT] build (nk,nj,8) packed stencil
            return np.stack([jm(bm1_n), bm1_n, jp(bm1_n),  # 0,1,2 (b-1, j-1/j/j+1) @ n   # [ROLLOUT]
                             jm(b_n),   b_n,   jp(b_n),    # 3,4,5 (b,   j-1/j/j+1) @ n   # [ROLLOUT]
                             bm1_np1, bm2_np1], axis=-1)   # 6,7   pim1, pim2 @ n+1       # [ROLLOUT]

        nsamp = 0; nrun = nsteps                                                       # [ROLLOUT]
        for it in range(1, nsteps + 1):                                                # [ROLLOUT]
            # snapshot phi^n at columns b-1, b (all rows j) BEFORE the step            # [ROLLOUT]
            h_bm1, h_b = self.h[:, :, b-1].copy(), self.h[:, :, b].copy()              # [ROLLOUT]
            u_bm1, u_b = self.u[:, :, b-1].copy(), self.u[:, :, b].copy()              # [ROLLOUT]
            v_bm1, v_b = self.v[:, :, b-1].copy(), self.v[:, :, b].copy()              # [ROLLOUT]
            # advance interior WITHOUT the njit OBC (obc_on=0); west prescribe as run_obc  # [ROLLOUT]
            self._step_core(dt, 2, prev_cols,                                          # [ROLLOUT]
                            h_bc_all[it-1], u_bc_all[it-1], v_bc_all[it-1],            # [ROLLOUT]
                            h_diff, u_diff, v_diff, obc_on=0)                          # [ROLLOUT]
            # post-step: pim1=(b-1)@n+1, pim2=(b-2)@n+1; overwrite col b per field     # [ROLLOUT]
            for fld, bm1_n, b_n, ext_all, fcode in (                                   # [ROLLOUT]
                    (self.h, h_bm1, h_b, h_ext_all, 0),                                # [ROLLOUT]
                    (self.u, u_bm1, u_b, u_ext_all, 1),                                # [ROLLOUT]
                    (self.v, v_bm1, v_b, v_ext_all, 2)):                               # [ROLLOUT]
                X8 = _X8(bm1_n, b_n, fld[:, :, b-1].copy(), fld[:, :, b-2].copy())     # [ROLLOUT] (nk,nj,8)
                Xf = X8.reshape(-1, 8)                     # [ROLLOUT] (m,8) flat batch of stencils
                rx, ry, inflow = _phase_inflow_np(Xf)      # [ROLLOUT] analytic (m,); inflow flag always
                # NN overrides (rx, ry) for h only; u, v keep the analytic estimate    # [ROLLOUT]
                if phase_fn is not None and fcode == 0:                                # [ROLLOUT]
                    rx, ry = phase_fn(Xf, fcode)           # [ROLLOUT] normalization done inside phase_fn
                upd = _obc_update_np(Xf, rx, ry, inflow, ext_all[it-1].reshape(-1), alpha_eff)  # [ROLLOUT]
                fld[:, :, b] = upd.reshape(self.nk, self.nj)   # [ROLLOUT] overwrite col b
                fld[:, :, b+1] = fld[:, :, b].copy()       # [ROLLOUT] ghost copy b -> b+1 (module convention)
                fld[:, :, b+2] = fld[:, :, b+1].copy()     # [ROLLOUT] ghost copy b+1 -> b+2 (module convention)

            self.v[:, 0, :] = 0.0   # [ROLLOUT] [NP] south wall: col-b overwrite can set v at j=0; re-zero
            if np.any(np.isnan(self.u)):                                               # [ROLLOUT]
                print('Model has blown up!!! Stopping early')                          # [ROLLOUT]
                u = u[:nsamp]; v = v[:nsamp]; h = h[:nsamp]; time = time[:nsamp]       # [ROLLOUT]
                nrun = it; break                                                       # [ROLLOUT]
            if it % samp == 0:                                                         # [ROLLOUT]
                nsamp += 1                                                             # [ROLLOUT]
                u[nsamp] = self.u; v[nsamp] = self.v                                   # [ROLLOUT]
                h[nsamp] = self.h; time[nsamp] = self.time                             # [ROLLOUT]
        print("...done")                                                               # [ROLLOUT]

        diffs = {'t_step': np.arange(1, nrun+1) * dt, 'h_max': None}                   # [ROLLOUT]
        return u, v, h, time, diffs                        # [ROLLOUT] 5-tuple: (u, v, h, time, diffs)
    ...
    def _step_core(self, dt, bc_mode, bc_cols,\
        ...
        # [OBC - 2D - PhaseDiag] per-row (rx,ry) recording arrays default to the shared scratch
