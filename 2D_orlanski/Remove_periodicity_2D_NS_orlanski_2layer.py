...
# This is a 2D scheme.
# [2L] 2-layer driver copy of OSSWEM_helpers/Remove_periodicity_2D_NS_orlanski.py.
...
# ===================== [2L][NN] out-of-step numpy OBC helpers =========================
def _phase_inflow_np(X8):
    """[2L][NN] Analytic per-point (rx, ry) + inflow flag from the packed stencil (numpy).
    Faithful copy of the estimation branch of _orlanski_east; inflow == (raw rx < 0)."""
    pim1, pim2            = X8[..., 6], X8[..., 7]
    ...
    eps_denom = 1e-5                             # [2L][NN] matches _orlanski_east above (frozen 1-layer value)
    r_d = 1.0 / np.maximum(denom, eps_denom)     # array-safe small-denominator guard
    rx = -dphi_t * dphi_x * r_d
    ry = -dphi_t * dphi_y * r_d
...
def _obc_update_np(X8, cx, cy, inflow, phi_ext, alpha_in, dphi_in=None, dphi_out=None):
    """[2L][NN] East-Orlanski boundary update (numpy), faithful to _orlanski_east.
    OUTFLOW: phi_b = (phi_prev_b + cx*pim1 - cy*dy_b)/(1+cx) [+ dphi_out].
    INFLOW : phi_b = phi_prev_b + alpha_in*(phi_ext - phi_prev_b) [+ dphi_in]
             (cx,cy unused; alpha_in=1 == hard prescribe, alpha_in=0 == persistence).
    NOTE: unlike the in-step _orlanski_east this applies NO rx/ry clamp -- whatever the
    network returns is used as-is (same behaviour as the 1-layer NN test notebook)."""
    ...
    if dphi_out is not None:                                 # optional learned outflow-radiate correction
        rad = rad + dphi_out
    nudge = phi_prev_b_j + alpha_in * (phi_ext - phi_prev_b_j)
    if dphi_in is not None:                                  # optional learned inflow-nudge correction
        nudge = nudge + dphi_in
...
class SSWEM:
    ...
    def run_record_bc(self, dt, samp, nsamps, bc_cols,
                      ...
                      store_centerline=False, j_center=None):    # optional per-step zonal center-line store
        """
        dt      - Time step [s]
        samp    - Steps between samples [steps]
        ...
        store_centerline - if True, ALSO store (u,v,h) every step along the whole   
                  zonal line at row j_center (all i). Default False.                 
        j_center - row index of the stored zonal line; default None -> nj//2         
                  (domain center). h/u share y=yh1[jc]; v is at y=yq1[jc].            
        ...
        AND a `probe` dict (None only when BOTH store_downstream and store_centerline  
        are False). With store_downstream it has keys 'cols', 'x_km', and 'h'/'u'/'v'
        (each (nsteps, nk, nj, n_probe)); with store_centerline it also has 'j_center',  
        per-field x/y axes and 'h_cl'/'u_cl'/'v_cl' (each (nsteps, nk, ni)).             
        The probe is read from the post-step state; downstream of the sponge the
        restoring is zero there, so that equals the pre-restoring phase used for
        the boundary stores.
        """
        ...
        # center-line setup: store the whole zonal row jc (all i) every step.
        if store_centerline:                                                       
            jc = self.nj // 2 if j_center is None else int(j_center)               
            if not (0 <= jc < self.nj):                                             
                raise ValueError(f"[CL] j_center {jc} out of range [0, {self.nj})") 
            h_cl_all = np.zeros((nsteps, self.nk, self.ni))    # (nsteps, nk, ni)
            u_cl_all = np.zeros((nsteps, self.nk, self.ni))    
            v_cl_all = np.zeros((nsteps, self.nk, self.ni))    
        ...
        for iter in range( 1, nsteps + 1 ):
            ...
            if store_centerline:                              # [OBC] post-step zonal center line (all i at row jc)
                h_cl_all[iter-1] = self.h[:, jc, :]          
                u_cl_all[iter-1] = self.u[:, jc, :]          
                v_cl_all[iter-1] = self.v[:, jc, :]          
            if np.any( np.isnan( self.u ) ):
                print('Model has blown up!!! Stopping early')
                ...
                if store_centerline:                          # [OBC] trim center line too
                    h_cl_all = h_cl_all[:iter-1]             
                    u_cl_all = u_cl_all[:iter-1]             
                    v_cl_all = v_cl_all[:iter-1]             
                break
            if iter % samp == 0:
                nsamp += 1
        ...
        # [OBC] bundle the optional probe (None when both diagnostics disabled) as the last return
        probe = None
        if store_downstream or store_centerline:              # [OBC] probe holds either/both diagnostics
            probe = {}                                       
        if store_downstream:
            probe.update({ 'cols': np.arange(probe_i0, probe_i0 + n_probe),
                           'x_km': self.xh1[probe_i0:probe_i0 + n_probe] / 1e3,
                           'h': h_probe_all, 'u': u_probe_all, 'v': v_probe_all })
        if store_centerline:                                  # [OBC] zonal center-line stores + per-field x/y axes
            probe.update({ 'j_center': jc,                                            
                           'x_km_h': self.xh1 / 1e3, 'x_km_u': self.xq1 / 1e3,         # h,v on xh1; u on xq1
                           'y_km_h': self.yh1[jc] / 1e3, 'y_km_v': self.yq1[jc] / 1e3,  
                           'h_cl': h_cl_all, 'u_cl': u_cl_all, 'v_cl': v_cl_all })      
    ...
    # [2L][NN] ---------------- Neural-network OBC driver ---------------------------
    # [2L][NN] Ported from OSSWEM_helpers/NP_OSSWEM_orlanski_2D_NN_1step.run_obc_nn.
    # [2L][NN] Out-of-step: the interior is advanced with the njit OBC OFF (obc_on=0),
    # [2L][NN] then column b is overwritten per field in numpy using (rx,ry) from
    # [2L][NN] phase_fn (the network) or from the analytic _phase_inflow_np fallback.
    # [2L][NN] nk-generic: stencils are (nk,nj,8) flattened k-major to (nk*nj,8), so at
    # [2L][NN] nk=2 the network sees layer 0's rows then layer 1's rows.
    def run_obc_nn(self, dt, samp, nsampes, prev_cols,
                   h_bc_all, u_bc_all, v_bc_all, b_obc,
                   h_ext_all, u_ext_all, v_ext_all,
                   phase_fn=None, nudging=True, alpha_in=1,
                   nn_field_codes=(0,),      # field codes the NN drives; (0,)=h-only, (0,1,2)=all fields
                   nn_inflow_corr=False,     # phase_fn -> (rx,ry,dphi_in); dphi_in added to the inflow nudge
                   nn_outflow_corr=False):   # phase_fn -> (rx,ry,dphi_out); dphi_out added to the outflow radiate
        """
            samp : steps between stored output frames
            phi_bc_all : per-step stored western prescribed value
            phi_ext_all : external forcing at the open boundary, (>=nsteps, nk, nj)
            phase_fn : the neural network, called as phase_fn(Xf, fcode) with
                       Xf a (nk*nj, 8) batch of packed stencils -> (rx, ry)
        """
        prev_cols = np.ascontiguousarray(np.asarray(prev_cols, dtype=np.int64).ravel())
        b = int(b_obc)
        ...
        if not (2 <= b < self.ni - 2):
            raise ValueError(f" b_obc={b} needs 2 <= b < ni-2={self.ni-2}")
        if np.any((prev_cols >= b - 2) & (prev_cols <= b)):
            raise ValueError(f" prescribed cols must avoid {{{b-2},{b-1},{b}}}; got {list(prev_cols)}")

        self._print_run_info(dt, nsteps)
        print(f"[2L][NN] east phase speed from "
        ...
        print(f"[2L][NN] nk={self.nk} layers; NN drives field codes {nn_field_codes} "
              f"(0=h, 1=u, 2=v) on every layer")
        print("Running (Neural-Network-OBC, non-periodic N/S walls)...")
        ...
        # [NP] tangential neighbours via free-slip mirror-clamp (south/north walls), NOT np.roll.
        ...
        # diagnostic mirroring run_obc's rx dict. Each (nsteps, nk, nj, 2) -> per LAYER too.
        _rxy_rows = [np.zeros((nsteps, self.nk, self.nj, 2)) for _ in range(3)]   # [2L][NN]

        nsamp = 0; nrun = nsteps
        for it in range(1, nsteps + 1):
            # snapshot phi^n at columns b-1, b (all rows j, all layers) BEFORE the step
            ...
            for fld, bm1_n, b_n, ext_all, fcode in (
                ...
                Xf = X8.reshape(-1, 8)                       # (nk*nj, 8) flat batch, k-major
                rx, ry, inflow = _phase_inflow_np(Xf)        # analytic fallback

                dphi_in = None                               # optional learned inflow-nudge correction
                dphi_out = None                              # optional learned outflow-radiate correction
                if phase_fn is not None and fcode in nn_field_codes:
                    if nn_inflow_corr:
                        ...
                        rx, ry = phase_fn(Xf, fcode)         # network overrides the analytic (rx,ry)
                    inflow = rx < 0.0
                _rxy_rows[fcode][it-1, :, :, 0] = np.asarray(rx).reshape(self.nk, self.nj)   # applied rx per (layer,row)
                _rxy_rows[fcode][it-1, :, :, 1] = np.asarray(ry).reshape(self.nk, self.nj)   # applied ry per (layer,row)
                upd = _obc_update_np(Xf, rx, ry, inflow, ext_all[it-1].reshape(-1), alpha_eff,
                                     dphi_in=dphi_in, dphi_out=dphi_out)
                fld[:, :, b] = upd.reshape(self.nk, self.nj)   # overwrite col b
                fld[:, :, b+1] = fld[:, :, b].copy()           # ghost copy, as in _orlanski_east
        ...
        diffs = {'t_step': np.arange(1, nrun+1) * dt, 'h_max': None,
                 # [2L][NN] per-row APPLIED (rx,ry) for the NN run, mirroring run_obc's rx dict.
